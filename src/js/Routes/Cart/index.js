...
const Items = ({items, click}) => (<div className="boxes-container">
  {
    items.map((item, key) => (<div className="box" key={key}>
      <div className="delete" onClick={() => click(key)}>
        <p>x</p>
      </div>
      <div className="left">
        <img src={boxes[item["box"]]["image"]}/>
        <p>
          Glaze : {glazes[item["glaze"]]}
        </p>
      </div>
      <div className="right">
        {
          Object.entries(item["choices"]).map(([key, value]) => (<div className="info" key={key}>
            <p className="type">
              {buns[key]["title"]}
            </p>
            <p className="price">
              $ {buns[key]["price"].toFixed(2)}
            </p>
            <p className="times">
              x
            </p>
            ...
            <p className="total">
              $ {(buns[key]["price"] * value).toFixed(2)}
            </p>
          </div>))
        }
      </div>
  ...
  render() {
    ...
    return (<div className="cart">
      ...
      <div className="buttons">
        ...
        {
          data !== null && <Link to={"#"} onClick={this.checkout} style={{
                textDecoration: 'none'
              }} id="order-button">
              CHECKOUT
            </Link>
