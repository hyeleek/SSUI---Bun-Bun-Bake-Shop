...
class Order extends Component {
  ...
  boxClick(newIndex) {
    this.setState({box: newIndex, capacity: boxes[newIndex]["size"], choices: {}});
  }

  openPopUp(currentItem) {
    this.setState({showPopup: true, popup: currentItem});
  }
  ...
  refresh = () => {
    ...
    this.setState({stage: 0, box: null, choices: {}, glaze: "1", capacity: null});
    startFresh();
  }
  ...
  render() {
    ...
    return (<div className="order">
      <Steps titles={steps.slice(0, -1)} stage={stage} click={this.handleClick} box={box} capacity={capacity}/>
      <div className="viewer">
        {
          box === null
            ? added
              ? <p>
                  Your buns have been added to cart!
                </p>
              : <p>
                  CHOOSE YOUR BOX SIZE
                </p>
            : <OrderChoice boxKey={box} items={bunsOnBox(choices)} deleteFromBox={this.deleteFromBox}/>
        }
      </div>
            ...
            ? <div className={[
                  "button", "increase", stage === 0
                    ...
                    : (capacity !== 0)
                      ? "disalbed"
                      : false
                ].join(" ")} onClick={stage === 2
                  ? this.handleCart
                  : this.increase} disabled={stage === 0
                  ? (box === null)
                    ? true
                    : false
                  : (capacity !== 0)
      ...
      <div className="options">
        {
          stage === 0
            ? Object.entries(boxes).map(([key, value]) => (<BoxOption key={value["key"]} index={value["key"]} number={value["size"]} imgSrc={value["image"]} click={this.boxClick} selected={value["key"] === box}/>))
            : null
        }
        {
          stage === 1
            ? <div className="buns-container">
                {Object.entries(buns).map(([key, value]) => (<BunOption key={value["title"]} imgSrc={value["image"]} title={value["title"]} name={key} price={value["price"]} add={this.addToBox} open={this.openPopUp} close={this.closePopUp}/>))}
              </div>
            : null
        }
        {
          stage === 2
            ? Object.entries(glazes).map(([key, value]) => (<GlazeOption key={key} index={key} imgSrc={value["image"]} click={this.glazeClick} selected={key === glaze}/>))
            : null
        }
        {
          stage === 3 && <div className="checkout" onClick={this.refresh}>
              <p>
                Start Fresh</p>
            </div>
        }
